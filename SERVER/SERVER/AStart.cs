 ...
 * 
 * Changes :
 * -The old built has a project inside to make a demonstration of the path in a simple gride, now it was cleaned and just the library is inside, no need to make any touche.
 * -the old built request the grid to be SQUARE, so what happen if you have a 5/8 gride ? it's not working, so with that built it's working with any dimention you like,
 *      because it's rendred to the nearest square value, for ex if your gride dimention is 5/8, the real gride behide the code is 8/8, and the box added is considered as a wall, it's that simple ^^
 *      so you dont have to worry about it :)
 *  -The old built take the gride information from a file that you must generate by hand, now you can past the gride that you must initialise in you code by an 2D array of byte (byte[,]), that containe the obstacles so you can link to the main function, so easy
 *  -The old built use a reversed position X,Y in it's gride, that mean the if you want to put an obstacle in the first line (0) and in the second column you should type gride[0,1] = 3, and in gride[x,y], the X is often refered to the columne, and Y the line
 *      so to add an obstacle with real coordinate you have to use gride[1,0] as we do always.
 *  -The code has been cleaned out by some other stuff no realy interesting.
 * ****************************************************
 * 
 * How to use this :
